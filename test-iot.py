...
if label == 1:
    ...
    try:
        ...
        for sw in switches:
            for line_num in [1, 2]: 
                cmd = f"""ovs-ofctl -O OpenFlow13 dump-flows {sw} | awk '{{match($0, /n_bytes=([0-9]+)/, b); match($0, /duration=([0-9.]+)/, d); if (b[1] != "" && d[1] != "" && b[1]/d[1] > 1000) print $0}}' | grep in_port | sed -n '{line_num}p'"""
                try:
        ...
        if commands:
            ...
            try:
                for file in [output_file, packets_file, bytes_file, feature_file, mac_src_file, duration_file]:
                    try:
